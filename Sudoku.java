 ...
 *
 * your name: Yusra Nasir and Misha Nasir 
 *
 */
...
public class Sudoku {    
    ...
    private boolean valueFound(int row, int coloumn, int value){
        for (int i = 0; i < 9; i++) {
            if(value == grid[row][i]){ // if vaue at coloumn is found
                return true; 
            }
            if(value == grid[i][coloumn]){ // if value at row is found 
                return true;
            }
        }
        return false; 
    ...
    private boolean solveRB(int n) {
        if(n > 80){ // checked previous cells - stopping condition 
            return true; 
        }
        // finding location of n 
        int row = n/9; // finding row and coloumn of where n is 
        int coloumn = n%9;
        int subColoumn = coloumn/3; 
        int subRow = row/3;
        if(valIsFixed[row][coloumn]){
            return solveRB(n+1);
        }
        //solveRB(n+1); // recursive call - called 81 times 
        for (int val = 1; val <= 9; val++) { // checking if val is located in cell
           // this.printGrid();
            if(!subgridHasVal[subRow][subColoumn][val] && !valueFound(row, coloumn, val)){ // if there is no value then placeval
                placeVal(val, row, coloumn); // place value 
                //this.printGrid();
                if(solveRB(n+1)){
                    return true; // it is solved 
                }
                removeVal(val, row, coloumn);
